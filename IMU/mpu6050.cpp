...
u8 mpu6050::Init(bool wait)
{
	...
	IIC_Write_Temp=6;
	mI2C->AddCommand(MPU6050_ADDRESS,MPU_CONFIG,&IIC_Write_Temp,1,0,0);
	IIC_Write_Temp=0x00;
...
u8 mpu6050::Update(bool wait,Vector3<int> *acc, Vector3<int> *gyro)
{
	...
	/*mI2C->AddCommand(MPU6050_ADDRESS,MPU_CONFIG,0,0,&mRegisterConfig,1);//获取CONFIGH寄存器状态 无法读取，虽然手册上写的可读可写 */
