			...
			if (ch == 0xAA){//帧头判断完成
				...
				if(ch == 0x00){//console
					...
					if(Calibration(data, data[3] + 4, check)){
						if (data[4] == 0xa0 && !mClockState ){//飞机锁定
							mClockState = true;
//								reply(data[2], mClockState);//返回锁状态
							return true;
						}
						else if (data[4] == 0xa1&& mClockState){//飞机解锁
							mClockState = false;
//								reply(data[2], mClockState);//返回锁状态
							return true;
						}
...
bool Communication::reply(u8 difference, u8 parameter){
	...
	data_to_send[0] = 0xAA;
	data_to_send[1] = 0xAA;
	data_to_send[2] = 0x02;//返回系统参数
	data_to_send[3] = 0x01;
	data_to_send[4] = parameter;
	...
	while(times--){
		if(nrf.NRF_SendPacket(data_to_send) == TX_DS){
			break;
		}
	}
	return true;
